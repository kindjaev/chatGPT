  ...
  return (
    <div className="bg-primaryPurple">
      <form
        ...
        className="flex justify-between  px-8 rounded-full border w-[70vw] text-xl shadow-md"
      >
        <input
          type="text"
        ...
        <button type="submit" className="text-2xl text-gray-500 ml-4 p-4">
          <FiSend />
        </button>
      </form>
          ...
          messages.map((item, i) => {
            return (
              <div key={i}>
                ...
                {/* <p>{item.ai}</p> */}
                <p>
                  {item.ai ? <TypeText data={item.ai} key={i} /> : "......"}
                </p>
              </div>
            );
          })}
